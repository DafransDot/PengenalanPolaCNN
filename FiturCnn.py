...
# Visualisasi fitur dari layer CNN tertentu
def visualisasi_fitur(model, image_path, layer_index=0):
    img = load_img(image_path, target_size=(64, 64))
    img = img_to_array(img)
    img = np.expand_dims(img, axis=0)
    img = img / 255.0

    # Membuat model baru yang keluarannya adalah layer yang diinginkan
    layer_output = model.layers[layer_index].output
    feature_extractor = tf.keras.Model(inputs=model.inputs, outputs=layer_output)

    # Mendapatkan hasil ekstraksi fitur dari layer tersebut
    features = feature_extractor.predict(img)

    # Tentukan jumlah filter pada layer
    filter_count = features.shape[-1]
    plt.figure(figsize=(15, 15))
    
    # Loop untuk menampilkan setiap filter
...
if __name__ == "__main__":
    ...
    # Visualisasi ekstraksi fitur di layer convolusi pertama (layer index 0)
    visualisasi_fitur(model, image_path, layer_index=0)
